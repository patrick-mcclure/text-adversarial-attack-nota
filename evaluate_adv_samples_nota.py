...
def evaluate_adv_samples(model, tokenizer, tokenizer_surr, adv_log_coeffs, clean_texts, labels,
                         ...
                         print_every=10, pretrained=False, pretrained_surrogate=False, label_perm=(lambda x: x), nota=False):
    ...
    with torch.no_grad():
        for i in tqdm(range(num_samples)):
            ...
            if (i+1) % print_every == 0:
                ...
                print('Adversarial success rate = %.4f' % torch.cat(all_asr, 0).float().mean(1).eq(1).float().mean())
    
    all_corr = torch.cat(all_corr, 0)
    all_asr = torch.cat(all_asr, 0)
    ...
    return all_sentences, all_corr, all_asr, cosine_sim
...
def main(args):
    ...
    all_sentences, all_corr, all_asr, cosine_sim = evaluate_adv_samples(
